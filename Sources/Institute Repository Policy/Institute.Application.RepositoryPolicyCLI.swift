...
extension Institute.Application {
    enum RepositoryPolicyCLI {}
}

extension Institute.Application.RepositoryPolicyCLI {
    ...
    static func execute(_ arguments: [Swift.String]) async {
        do {
            let operation = arguments.first
            let rest = Array(arguments.dropFirst())
            if operation == "validate" {
                try validate(ValidationArguments(rest))
            } else if operation == "compact" {
                try await compact(CompactionArguments(rest))
            } else if operation == "ruleset" {
                try ruleset(RulesetArguments(rest))
            } else if operation == "render-caller" {
                try renderCaller(rest)
            } else if operation == "parse-caller" {
                try parseCaller(rest)
            } else if operation == "draft-metadata" {
                try draftMetadata(rest)
            } else if operation == "census" {
                try census(rest)
            } else if operation == "capability-records" {
                try capabilityRecords(rest)
            } else if operation == "ruleset-convergence" {
                try rulesetConvergence(
                    RulesetConvergenceArguments(rest)
                )
            } else if operation == "caller-wave" {
                try await callerWave(rest)
            } else if operation == "uniformity-wave" {
                try await uniformityWave(rest)
            } else {
                try await reconcile(ReconcileArguments(["institute", "repository"] + arguments))
            }
        } catch {
            FileHandle.standardError.write(Data("institute repository: \(error)\n".utf8))
            Institute.Application.CI.terminate(1)
        }
    }
    ...
    private static func compact(_ arguments: CompactionArguments) async throws(Error) {
        ...
        guard let plan else {
            print("institute repository: compact already-converged")
            return
        }
        print(
            "institute repository: compact \(arguments.apply ? "applied" : "dry-run") "
    ...
    private static func reconcile(_ arguments: ReconcileArguments) async throws(Error) {
        ...
        let excludedDescription =
            ...
            : receipt.excluded.sorted { $0.key < $1.key }
                ...
                .joined(separator: ",")
        print(
            "institute repository: scope=\(receipt.scope) examined=\(receipt.examined) "
