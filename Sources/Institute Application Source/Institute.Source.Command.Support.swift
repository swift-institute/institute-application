...
extension Institute.Source.Command {
    ...
    static func write(
        ...
        if file.stat.exists {
            switch format {
            case .json:
                let existing: Source.Report
                let contents = try read(output)
                do throws(JSON.Error) { existing = try .init(jsonString: contents) }
                catch { throw .configuration("existing source report is malformed: \(error)") }
    ...
    private static func outputFile(_ output: Swift.String) throws(Institute.Error) -> File {
        ...
        do throws(File.Path.Error) { return try File(.init(output)) }
        catch { throw .configuration("invalid artifact path \(output)") }
    }

    private static func atomic(_ contents: Swift.String, to file: File) throws(Institute.Error) {
        do throws(File.System.Write.Atomic.Error) { try file.write.atomic(contents) }
        catch { throw .filesystem("cannot write artifact \(file): \(error)") }
    }
}
